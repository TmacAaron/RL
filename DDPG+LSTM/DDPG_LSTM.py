...
class ActorNet(nn.Module):
    def __init__(self, lr_a, input_dims, lstm_dims, fc1_dims, fc2_dims, n_actions, name, chkpt_dir='tmp/ddpg'):
        ...
        self.lstm = nn.LSTM(input_size=self.input_dims, hidden_size=lstm_dims, num_layers=1)
        self.fc1 = nn.Linear(lstm_dims, self.fc1_dims)
        f1 = 1 / np.sqrt(self.fc1.weight.data.size()[0])
        torch.nn.init.uniform_(self.fc1.weight.data, -f1, f1)
        torch.nn.init.uniform_(self.fc1.bias.data, -f1, f1)
        self.bn1 = nn.LayerNorm(self.fc1_dims)
...
class Agent(object):
    def __init__(self, lr_c, lr_a, input_dims, lstm_dims, tau, env, gamma=0.99, n_actions=2,
                 max_size=100000, layer1_size=400, layer2_size=300, batch_size=64, chkpt_dir='tmp/ddpg'):
        self.gamma = gamma
        self.tau = tau
        self.memory = ReplayBuffer(max_size, input_dims, n_actions, lstm_dims)
        self.batch_size = batch_size

        self.actor = ActorNet(lr_a, input_dims, lstm_dims, layer1_size, layer2_size, n_actions, name='Actor', chkpt_dir=chkpt_dir)
        self.target_actor = ActorNet(lr_a, input_dims, lstm_dims, layer1_size, layer2_size, n_actions, name='TargetActor', chkpt_dir=chkpt_dir)
        self.critic = CriticNet(lr_c, input_dims, layer1_size, layer2_size, n_actions, name='Critic', chkpt_dir=chkpt_dir)
        self.target_critic = CriticNet(lr_c, input_dims, layer1_size, layer2_size, n_actions, name='TargetCritic', chkpt_dir=chkpt_dir)

        self.noise = OUActionNoise(mu=np.zeros(n_actions))

        self.update_target_net_parameters(tau=1)

    def choose_action(self, observation, hc0, evaluate=False):
        h0, c0 = hc0
        self.actor.eval()
        ...
        mu, (h, c) = self.actor.forward(observation, (h0, c0))
        mu = mu.to(self.actor.device)
        mu = mu.reshape(-1)
        h = h.to(self.actor.device)
        c = c.to(self.actor.device)
        if evaluate:
            mu_prime = mu
        else:
        ...
        return mu_prime.cpu().detach().numpy(), (h.cpu().detach().numpy(), c.cpu().detach().numpy())
    ...
    def learn(self):
        ...
        target_actions_, _ = self.target_actor.forward(b_next_state, (b_hn, b_cn))
        ...
        b_mu, _ = self.actor.forward(b_state, (b_h0, b_c0))
        self.actor.train()
