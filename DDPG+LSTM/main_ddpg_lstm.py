...
agent = Agent(lr_c=0.0025, lr_a=0.001, input_dims=input_dims, lstm_dims=100, tau=0.001, env=env,
              batch_size=64, layer1_size=400, layer2_size=300, n_actions=n_actions, chkpt_dir=chkpt_dir)
if resume:
    agent.load_models()
...
for e in bar:
    ...
    while not done:
        env.render()
        act, (h, c) = agent.choose_action(state, (h0, c0))
    ...
    if e % 50 == 0:
        ...
        for i in eval_bar:
            ...
            while not done:
                env.render()
                act, (h, c) = agent.choose_action(state, (h0, c0), evaluate=True)
                next_state, reward, done, _ = env.step(act)
                score += reward
                state = next_state
                h0, c0 = h, c
